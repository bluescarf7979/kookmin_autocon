...
def lane_tracking_drive(speed):
    ...
    while True:
        ...
        mission_flag = check_cone_mission()
        cnt += 1
...
def check_cone_mission():
    global lidar_points, MIN_DISTANCE, MAX_DISTANCE

    front_lidar_data = lidar_points[(505 * 3 / 4):] + lidar_points[:(505 / 4)]
    front_lidar_data = np.array(front_lidar_data) * 100 * a
    front_lidar_data[front_lidar_data <= MIN_DISTANCE] = 0.0
    front_lidar_data[front_lidar_data >= MAX_DISTANCE] = 0.0

    mission_check = np.sum((front_lidar_data > 5 * a) & (front_lidar_data < 60 * a))
    # print(mission_check)
...
def cone():
    ...
    global a, cnt, prev_steer

    while True:
        ACT_RAD = np.int32(ACTUAL_RADIUS)
        front_lidar_data = lidar_points[505 * 3 / 4:] + lidar_points[:505 / 4]  # front_lidar_data : 253
        # front_lidar_data = lidar_points[505 * 3 / 4:]
        front_lidar_data = np.array(front_lidar_data) * 100 * a  # Àü¹æ ÃøÁ¤ ½ÃÀÛ, ¹Ý½Ã°è ¹æÇâÀ¸·Î 360µµ, 505°³ data
        front_lidar_data[front_lidar_data <= MIN_DISTANCE] = 0.0
        front_lidar_data[front_lidar_data >= MAX_DISTANCE] = 0.0
        # print("LEN:", len(front_lidar_data))
        # print("FRONT:", front_lidar_data)

        mission_cone = check_cone_mission()
        ...
        if mission_cone:
            ...
            # interval = 180 / (253 - 1)
            # ####original###
            angle = [float(i * 180 / (253 - 1)) for i in range(len(front_lidar_data))]
            ...
            if current_frame is not None:
                for theta in range(ARC_ANGLE + 1):
                    ...
                    for r in range(ACT_RAD):
                        ...
                        if (current_frame[y][x] != 0) and theta != ARC_ANGLE:
                            data[theta][0] = 1
                            data[theta + 1][0] = 1
                        elif (current_frame[y][x] != 0) and theta == ARC_ANGLE:
                            data[theta][0] = 1
                ...
                # if target >= 0:
                #     x_target = ACT_RAD + int(
                #         data_transposed[1][int(target) - AUX_RANGE] * np.cos(np.radians(int(target))))
                #     y_target = ACT_RAD - int(
                #         data_transposed[1][int(target) - AUX_RANGE] * np.sin(np.radians(int(target))))
...
# 0.8, 40
            ...
            # if (prev_steer - 5 > steer) or (prev_steer + 5 < steer):
            #     steer = prev_steer
            # prev_steer = steer
